...
class SoftNLLLoss(nn.Module):
    ...
    def forward(self, logprobs, y):
        ...
        nll_loss = -logprobs.gather(dim=1, index=y_masked.unsqueeze(1).long())  # Convert y_masked to int64
...
def start_logging(args, lr, n_epochs, patience):
    ...
    if len(sys.argv) == 3 and sys.argv[2] == "cluster":
        ...
        runs_path = 'runs'
    print(runs_path, args['alias'], args['architecture'])
    
    if not os.path.exists(f"{runs_path}/{args['alias']}"):
        os.makedirs(f"{runs_path}/{args['alias']}/", exist_ok=True)
    if not os.path.exists(f"{runs_path}/{args['alias']}/{args['architecture']}"):
        os.makedirs(f"{runs_path}/{args['alias']}/{args['architecture']}", exist_ok=True)
